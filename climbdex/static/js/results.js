...
async function fetchGenerateResults() {
    ...
    const response = await fetch("/backend/generate", {
        ...
        body: JSON.stringify({
            ...
            "layout_id": layout_id,
            "reach": Math.max(40.7, reach / 20.32 * 8 * (1 - 0.4 * angle / 70)),
            "power": power,
...
function drawResultsPage(results, pageNumber, pageSize, resultsCount) {
    ...
    if (!generate) resultsList.onscroll = function (event) {
...
if (!generate) {
    ...
    fetchGenerateResults().then((results) => {
        const resultsCountHeader = document.getElementById("header-results-count");
        resultsCountHeader.textContent = `Generated ${results.length} climbs`;
        drawResultsPage(results, 0, results.length, results.length);
    });
}
